    ...
    def test(self, iters=None, **kwargs):
        ...
        if iters is not None:
            ...
            for i in range(iters):
                for traj in self.rollout(train_listener=False, **kwargs):
                    self.loss = 0
...
class Seq2SeqAgent(BaseAgent):
    ...
    def _sort_batch(self, obs, probs=None):
        ...
        if probs is not None:
            seq_probs = probs[perm_idx]
            return Variable(sorted_tensor, requires_grad=False).long().cuda(), \
                   mask.cuda(), \
                   list(seq_lengths), list(perm_idx), seq_probs

        return Variable(sorted_tensor, requires_grad=False).long().cuda(), \
               mask.cuda(),  \
               list(seq_lengths), list(perm_idx), None
    ...
    def rollout(self, train_ml=None, train_rl=True, reset=True, speaker=None, train_speaker=False, train_listener=True, last_env_batch=None, get_probs=False, straight_through=False):
        """
        ...
        if last_env_batch is not None:
            obs = np.array(self.env.reset(last_env_batch))
        else:
        ...
        if speaker is not None:         # Trigger the self_train mode!
            ...
            insts, _, probs, speaker_loss = speaker.infer_batch(featdropmask=noise, train=train_speaker, get_probs=get_probs)     # Use the same drop mask in speaker
        ...
        seq, seq_mask, seq_lengths, perm_idx, seq_probs = self._sort_batch(obs, probs=probs)
        perm_obs = obs[perm_idx]

        ctx, h_t, c_t = self.encoder(seq, seq_lengths, seq_probs=seq_probs, straight_through=straight_through)
        ctx_mask = seq_mask
