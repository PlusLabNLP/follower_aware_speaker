...
class EncoderLSTM(nn.Module):
    ...
    def forward(self, inputs, lengths, seq_probs=None, straight_through=False):
        ''' Expects input vocab indices as (batch, seq_len). Also requires a 
            list of lengths for dynamic batching. '''
        embeds = self.embedding(inputs)  # (batch, seq_len, embedding_size)
        if seq_probs is not None and straight_through:
            emb_W = self.embedding.weight
            prob_emb = torch.matmul(seq_probs[:, :-1, :], emb_W)
            embeds[:, 1:-1, :] = embeds[:, 1:-1, :].detach() + prob_emb - prob_emb.detach()
