...
def train(train_env, tok, n_iters, log_every=100, val_envs={}, aug_env=None):
    ...
    for idx in range(start_iter, start_iter+n_iters, log_every):
        ...
        if aug_env is None:     # The default training process
            ...
            if args.accumulate_grad:
                for _ in range(interval // 2):
                    listner.zero_grad()
                    speaker.zero_grad()
                    listner.encoder.train()
                    listner.decoder.train()
                    listner.critic.train()
                    ...
                    for env_name in val_env_names:
                        env, evaluator = val_envs[env_name]
                        listner.env = env
                        listner.accumulate_gradient(feedback_method)
                        listner.loss.backward()
                        import copy
                        last_env_batch = copy.deepcopy(listner.last_env_batch)
                        old_env_batch_dev.append(last_env_batch)
                        old_grads = []
                        for group in listner.encoder_optimizer.param_groups:
                            for p in group['params']:
                                if p.grad is None:
                                    continue
                                old_grads.append(p.grad.clone().view(-1))
                        for group in listner.decoder_optimizer.param_groups:
                            for p in group['params']:
                                if p.grad is None:
                                    continue
                                old_grads.append(p.grad.clone().view(-1))
                        old_grads = torch.cat(old_grads)
                        old_grads_dev.append(old_grads)
                    ...
                    speaker.env = aug_env
                    listner.env = aug_env
                    listner.accumulate_gradient(feedback_method, speaker=speaker, train_speaker=True, train_listener=True, get_probs=False, straight_through=True)
                    loss = listner.loss + listner.speaker_loss
                    loss.backward()
                    listner.optim_step(backward=False)
                    ...
                    for env_name in val_env_names:
                        ...
                        args.ml_weight = 0.2
                        listner.env = env
                        listner.accumulate_gradient(feedback_method, last_env_batch=old_env_batch_dev[env_cnt])

                        listner.loss.backward()
                        loss_new = listner.loss.detach()
                        new_losses.append(loss_new)
                        ...
                        for group in listner.encoder_optimizer.param_groups:
                            for p in group['params']:
                                if p.grad is None:
                                    continue
                                new_grads.append(p.grad.clone().view(-1))
                        for group in listner.decoder_optimizer.param_groups:
                            for p in group['params']:
                                if p.grad is None:
                                    continue
                        ...
                        new_grads_dev.append(new_grads)

                        dot_product = torch.nn.functional.cosine_similarity(old_grads_dev[env_cnt].view(1, -1), new_grads.view(1, -1))
                        dot_products = dot_products + dot_product

                        env_cnt += 1
                        
                    env, evaluator = val_envs['train']
                    listner.zero_grad()
                    args.ml_weight = 0.2
                    listner.env = env
                    listner.accumulate_gradient(feedback_method)
                    ...
                    dot_product = dot_products / env_cnt
                    for group in speaker.encoder_optimizer.param_groups:
                        for p in group['params']:
                            if p.grad is None:
                                continue
                            p.grad *= dot_product
                    for group in speaker.decoder_optimizer.param_groups:
                        for p in group['params']:
                            if p.grad is None:
                                continue
                            p.grad *= dot_product
                    ...
                    speaker.env = train_env
                    speaker.accumulate_gradient()   # Train interval iters
                    speaker.optim_step()
            else:
                for _ in range(interval // 2):
                    # Train with GT data
        ...
        for env_name in best_val:
            if best_val[env_name]['update']:
                ...
                if speaker is not None:
                    speaker.save(idx, os.path.join("snap", args.name, "state_dict", "speaker_best_%s" % (env_name)))
        ...
        if iter % 50000 == 0:
            ...
            if speaker is not None:
                speaker.save(idx, os.path.join("snap", args.name, "state_dict", "speaker_Iter_%06d" % (iter)))
...
def train_val():
    ...
    if args.train == 'validspeaker':
        val_env_names = ['aug', 'val_unseen', 'val_seen']
    else:
