...
class Speaker():
    ...
    def accumulate_gradient(self, train_eval=None, last_env_batch=None):
        import copy
        if last_env_batch is not None:
            self.env.reset(last_env_batch)
        else:
            self.env.reset()
            self.last_env_batch = copy.deepcopy(self.env.batch)
        self.loss += self.teacher_forcing(train=True)
    
    def optim_step(self, backward=True):
        if backward:
            self.loss.backward()
    ...
    def get_insts(self, wrapper=(lambda x: x)):
        ...
        for _ in wrapper(range(total // self.env.batch_size + 1)):  # Guarantee that all the data are processed
            obs = self.env.reset()
            insts, _, _, _ = self.infer_batch()  # Get the insts of the result
    ...
    def infer_batch(self, sampling=False, train=False, featdropmask=None, get_probs=False):
        """
        ...
        if not train or get_probs:
            with torch.no_grad():
                import copy
                self.eval_encoder = copy.deepcopy(self.encoder)
                self.eval_decoder = copy.deepcopy(self.decoder)
                self.eval_encoder.eval()
                self.eval_decoder.eval()
                eval_ctx = self.eval_encoder(can_feats, img_feats, lengths,
                                   already_dropfeat=(featdropmask is not None))
                words = []
                gpu_words = []
                h_t = torch.zeros(1, batch_size, args.rnn_dim).cuda()
                ...
                for i in range(args.maxDecode):
                    # Decode Step
                    logits, h_t, c_t = self.eval_decoder(word, eval_ctx, ctx_mask, h_t, c_t)      # Decode, logits: (b, 1, vocab_size)
            ...
            del self.eval_encoder
            del self.eval_decoder
            if not train:
                return np.stack(words, 1), None, None, None
        ...
        for i in range(args.maxDecode):
            ...
            if not get_probs:
                ...
                if train:
                    ...
                    hidden_states.append(probs.squeeze())
                    entropies.append(m.entropy())
                else:
                    log_probs.append(log_prob.detach())
                    hidden_states.append(probs.squeeze().detach())
            ...
            gpu_ended = torch.tensor(~ended)
            speaker_denom += torch.sum(gpu_ended)
            speaker_loss += torch.sum(-log_prob[gpu_ended])
            
            # End?
            ended = np.logical_or(ended, cpu_word == self.tok.word_to_index['<EOS>'])
            if ended.all():
                break
        speaker_loss = speaker_loss/(speaker_denom+1e-9)
        return np.stack(words, 1), torch.stack(log_probs, 1), torch.stack(hidden_states, 1), speaker_loss #torch.stack(entropies, 1)
